...
{
    public class SRTDevilMayCry4DirectX : PluginBase, IPluginUI
    {
        internal static PluginInfo _Info = new PluginInfo();
        public override IPluginInfo Info => _Info;
        public string RequiredProvider => "SRTPluginProviderDMC4SE";
        private IPluginHostDelegates hostDelegates;
        private IGameMemoryDMC4SE gameMemory;
        ...
        private SolidBrush _devil;
        private SolidBrush _deviltext;

        public PluginConfiguration config;
        private Process GetProcess() => Process.GetProcessesByName("DevilMayCry4SpecialEdition")?.FirstOrDefault();
        ...
        public override int Startup(IPluginHostDelegates hostDelegates)
        {
            ...
            _devil = _graphics?.CreateSolidBrush(128, 0, 128, 100);
            _deviltext = _graphics?.CreateSolidBrush(222, 182, 255);
        ...
        {
            this.gameMemory = (IGameMemoryDMC4SE)gameMemory;
        ...
        private void DrawOverlay()
        {
            ...
            if (config.Debug)
            {
                DrawTextBlock(ref textOffsetX, ref statsYOffset, config.VersionInfoString, gameMemory.VersionInfo);
                DrawTextBlock(ref textOffsetX, ref statsYOffset, config.GameInfoString, gameMemory.GameInfo);
            }

            // DRAW PLAYER HP BAR
            if (config.ShowHPBars)
            {
                DrawHealthBar(ref statsXOffset, ref statsYOffset, gameMemory.Player.Name, gameMemory.Player.CurrentHP, gameMemory.Player.MaxHP, gameMemory.Player.PercentageHP);
                DrawDevilTriggerBar(ref statsXOffset, ref statsYOffset, gameMemory.Player.CurrentDT, gameMemory.Player.MaxDT, gameMemory.Player.PercentageDT);
            }
            else
            {
                DrawTextBlock(ref textOffsetX, ref statsYOffset, gameMemory.Player.Name, string.Format("{0} / {1}   {2:P1}", gameMemory.Player.CurrentHP, gameMemory.Player.MaxHP, gameMemory.Player.PercentageHP));
                DrawTextBlock(ref textOffsetX, ref statsYOffset, config.DevilTriggerString, string.Format("{0} / {1}   {2:P1}", gameMemory.Player.CurrentDT, gameMemory.Player.MaxDT, gameMemory.Player.PercentageDT));
            }
            ...
            foreach (EnemyHP enemy in gameMemory.EnemyHealth)
            {
                if (enemy.IsAlive)
                {
                    if (config.ShowHPBars)
                    ...
                    {
                        DrawTextBlock(ref xOffset, ref yOffset, "", string.Format("{0} / {1}   {2:P1}", enemy.CurrentHP, enemy.MaximumHP, enemy.Percentage));
                    }
                }
            }
        }
        ...
        private void DrawDevilTriggerBar(ref float xOffset, ref float yOffset, float chealth, float mhealth, float percentage = 1f)
        {
            string perc = float.IsNaN(percentage) ? "0%" : string.Format("{0:P1}", percentage);
            float endOfBar = config.PositionX + 420f - GetStringSize(perc);
            _graphics.DrawRectangle(_greydark, xOffset, yOffset += 28f, xOffset + 420f, yOffset + 22f, 4f);
            _graphics.FillRectangle(_greydarker, xOffset + 1f, yOffset + 1f, xOffset + 418f, yOffset + 20f);
            _graphics.FillRectangle(_devil, xOffset + 1f, yOffset + 1f, xOffset + (418f * percentage), yOffset + 20f);
            _graphics.DrawText(_consolasBold, 20f, _deviltext, xOffset + 10f, yOffset - 2f, string.Format("{0}{1} / {2}", "Devil Trigger: ", chealth, mhealth));
            _graphics.DrawText(_consolasBold, 20f, _deviltext, endOfBar, yOffset - 2f, perc);
        }
    }
}
